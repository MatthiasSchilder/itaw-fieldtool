...
{
    public partial class AccVisualizer : PictureBox
    {
        private Color _noDataColor;
        private const int LineHeightPx = 15;
        private const int LineHeightWoMarginPx = 13;

        public AccVisualizer()
        {
            InitializeComponent();
        }

        public void Setdata(Color noDataColor, Dictionary<DateTime, double[]> result)
        {
            _noDataColor = noDataColor;
            this.Image = RenderBurstActivitiesToBitmap(result);
            this.AutoSize = true;
        }

        private Bitmap RenderBurstActivitiesToBitmap(Dictionary<DateTime, double[]> result)
        {
